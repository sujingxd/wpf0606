...
namespace Aga.Diagrams
{
	public class DiagramView : Canvas
	{
        ...
        private static TextBox compileBox;
        static bool bCompileBoxAdded = false;
		...
		static DiagramView()
		{
            ...
            compileBox = new TextBox();
            //  compileBox.Margin = new Thickness(600, 20, 0, 0);
            compileBox.Margin = new Thickness(600, 0, 0, 0);
            compileBox.Height = 1000;
            compileBox.Width = 800;
		}

        public void ShowCompileBox(string strCodeText)
        {
            if (!bCompileBoxAdded)
            {
                this.Children.Add(compileBox);
                bCompileBoxAdded = true;
            }
            compileBox.Visibility = Visibility.Visible;
            compileBox.Text = strCodeText;
            compileBox.BringIntoView();
        }

        public void HideCompileBox()
        {
            compileBox.Visibility = Visibility.Collapsed;
        }
